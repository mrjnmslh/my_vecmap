...
def main():
    ...
    parser.add_argument('--output', default='res.txt', type=str, help='res dictionary')
    ...
    srcfile = io.open(args.src_embeddings,encoding=args.encoding)
    trgfile = io.open(args.trg_embeddings,encoding=args.encoding)


    src_words, x = embeddings.read_new(srcfile,threshold=200000, dtype=dtype)
    trg_words, z = embeddings.read_new(trgfile,threshold=200000, dtype=dtype)
    ...
    res = []
    for i in src:
        res.append('{} {}'.format(src_words[i],trg_words[translation[i]]))
