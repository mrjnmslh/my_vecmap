...
#                self.vectors = normalize(self.vectors)
    ...
    def load_from_word2vec_new(self,dataFile,full_vocab=False):
        vectors = []
        if full_vocab:
            self.max_vocab=9e8
    ...
    def save_transformed_vectors2(self, filename):
        print("number of embeddings {}".format(len(self.transformed_vectors)))
        with io.open(filename, 'w',encoding='utf-8') as fout:
            fout.write('{} {}\n'.format(self.num_words,self.embedding_dim).decode('utf-8'))
            for i in range(self.num_words):
                fout.write(self.words[i] + ' ' + ' '.join(str(x) for x in \
                                                           self.transformed_vectors[i]) + '\n')
