...
def main():
    ...
    parser.add_argument('--cuda', default=True ,action='store_true', help='use cuda (requires cupy)')
    ...
    parser.add_argument('--device_id', default=3, type=int, help='device ID used for the --cuda option (defaults to 0)')
    ...
    self_learning_group.add_argument('--vocab', default=200000, type=int , help='threthold of words')
    args = parser.parse_args()


    if args.unsupervised or args.acl2018:
        parser.set_defaults(init_unsupervised=False, unsupervised_vocab=4000, normalize=['unit', 'center', 'unit'], whiten=True, src_reweight=0.5, trg_reweight=0.5, src_dewhiten='src', trg_dewhiten='trg', self_learning=True, vocabulary_cutoff=20000, csls_neighborhood=10)
    ...
    srcfile= codecs.open(args.src_input, 'r', encoding='utf-8', newline='\n', errors='ignore')
    #srcfile = open(args.src_input, encoding=args.encoding, errors='surrogateescape',"ignore")
    #trgfile = open(args.trg_input, encoding=args.encoding, errors='surrogateescape',"ignore")
    trgfile= codecs.open(args.trg_input, 'r', encoding='utf-8', newline='\n', errors='ignore')
    src_words, x = embeddings.read_new(srcfile, args.vocab, dtype=dtype)
    trg_words, z = embeddings.read_new(trgfile, args.vocab, dtype=dtype)
    ...
    if args.init_dictionary:
        ...
        src_indices = src_indices[:args.unsupervised_vocab]
        trg_indices = trg_indices[:args.unsupervised_vocab]
    ...
    while True:
        ...
        if end:
            ...
            print('update the training dictionary...')
    ...
    if args.validation is not None:
        ...
        nn = asnumpy(simval.argmax(axis=1))
        accuracy = np.mean([1 if nn[i] in validation[src[i]] else 0 for i in range(len(src))])
        print('Final accuracy (without normalized):{}'.format(accuracy))
